...
//     cout << "segmeents initiaux" << segments->nb << endl;
...
//     do
//     {
//         sprintf(line_tmp, "%4d TRI3 1   1  11  11     %2d     %2d     %2d", i, tmp_el->segment1->node1->id, tmp_el->segment1->node2->id, tmp_el->segment2->node2->id);
//         str_tmp = line_tmp;
...
int main()
{
    ...
    int i;
    for(i=0; i<5; i++)
        addElement(nodes, initNode((float)i, 0.0f, i));

    insertElement(nodes, initNode(9.0f, 0.0f, 0), 3);

    bool run = true;

    for(node = nodes->first; run; node=node->next)
    {
        cout << "id: " << node->x << endl;
    ...
    cout << "yolo " << getElement(nodes, nodes->last->id)->id << endl;
